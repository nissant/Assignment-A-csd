...
function renderCoffees(coffees) {
  ...
  coffees.forEach((coffee) => {
    ...
    col.innerHTML = `
      <article class="coffee-item h-100">
        <img src="${coffee.image}" alt="${coffee.name}" />
        <div class="coffee-content">
          <h3 class="h6 mb-1">${coffee.name}</h3>
          <p class="coffee-price mb-2">${coffee.price}</p>
          <button class="btn btn-sm btn-order w-100">Add to Order</button>
        </div>
      </article>
    `;
...
if (orderList) {
  renderCoffees(coffeeMenu);

  if (searchInput) {
    searchInput.addEventListener("input", (e) => {
      const searchTerm = e.target.value.toLowerCase().trim();
      const filtered = coffeeMenu.filter((coffee) =>
